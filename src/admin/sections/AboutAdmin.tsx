...
import { aboutVideoExists, getAboutVideoUrl, uploadAboutVideo } from '../../lib/cms';
...
export default function AboutAdmin() {
  ...
  const fetchCurrent = async () => {
    setLoading(true);
    const exists = await aboutVideoExists();
    setHasVideo(exists);
    setVideoUrl(getAboutVideoUrl());
    setLoading(false);
  };
  ...
  const handleConfirmUpload = async () => {
    ...
    setUploading(true);
    try {
      await uploadAboutVideo(fileToUpload);
      setHasVideo(true);
      setVideoUrl(getAboutVideoUrl());
      setUploaded(true);
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-serif text-3xl font-light italic text-[#1C1B1B]">About Us Video</h2>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-3 lg:sticky lg:top-8 self-start">
          ...
          <div className="relative aspect-[4/3] bg-[#1C1B1B] rounded-xl overflow-hidden border border-[#D9BDD0]/30">
            {hasVideo ? (
              <video
                key={videoUrl}
                src={videoUrl}
        ...
        <div className="bg-white border border-[#D9BDD0]/30 rounded-2xl p-6 shadow-sm space-y-4 self-start">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-sans text-xs font-semibold text-[#912A55] uppercase tracking-widest">About Us Video</h3>
          </div>
      ...
      <ConfirmModal
        ...
        message={
          pendingFile
            ? `Are you sure you want to replace the current About Us video with "${pendingFile.name}"?`
